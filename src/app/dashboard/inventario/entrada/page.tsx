...
import { mockRawMaterials, mockAreas } from '@/lib/mock-data';
...
export default function EntradaMercanciaPage() {
    ...
    const [areaId, setAreaId] = useState('area-almacen');
    ...
    const selectedItemData = mockRawMaterials.find(i => i.id === selectedItem);

    // Auto-llenar costo actual cuando se selecciona item
    useEffect(() => {
        if (selectedItemData) {
            setUnit(selectedItemData.baseUnit);
            setUnitCost(selectedItemData.costPerUnit || 0);
        }
    }, [selectedItemData]);
    ...
    const addItem = () => {
        ...
        const item = mockRawMaterials.find(i => i.id === selectedItem);
        if (!item) return;
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Simular envío (en producción, llamar a registrarEntradaMercancia)
            await new Promise(resolve => setTimeout(resolve, 1000));

            // Log de lo que se enviaría
            console.log('📦 ENTRADA DE MERCANCÍA:', {
                referencia: referenceNumber,
                documento: uploadedFile?.url,
                area: areaId,
                items: entradaItems,
                total: totalEntrada,
                registradoPor: user?.firstName,
            });

            setResult({
                success: true,
                message: `Entrada registrada: ${entradaItems.length} item(s), Total: ${formatCurrency(totalEntrada)}`,
            });
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-700/50">
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        >
                                            <option value="UNIT">Sacos</option>
                            ...
                            {selectedItem === 'ins-leche' && unit === 'UNIT' && quantity > 0 && (
                                <p className="mt-2 text-xs text-blue-600">
                                    💡 {quantity} saco(s) = {quantity * 20} litros
                                </p>
                            )}

                            {/* Indicador de cambio de costo */}
                            {selectedItemData && unitCost > 0 && selectedItemData.costPerUnit &&
                                unitCost !== selectedItemData.costPerUnit && (
                                    <p className={cn(
                                        'mt-2 text-xs',
                                        unitCost > selectedItemData.costPerUnit ? 'text-red-500' : 'text-emerald-500'
                                    )}>
                                        {unitCost > selectedItemData.costPerUnit ? '📈' : '📉'}
                                        Costo anterior: ${selectedItemData.costPerUnit?.toFixed(2)} →
                                        Nuevo: ${unitCost.toFixed(2)}
                                        ({((unitCost - selectedItemData.costPerUnit) / selectedItemData.costPerUnit * 100).toFixed(1)}%)
                                    </p>
                                )}
                        </div>
